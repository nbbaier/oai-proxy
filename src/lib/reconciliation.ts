...
import { getCurrentDate, incrementTokens, getUsageRecord } from './database';
...
export async function fetchOpenAIUsage(
  ...
  do {
    ...
    if (isDebugMode()) {
      ...
      console.log('Using admin key:', key.substring(0, 10) + '...' + key.substring(key.length - 4));
    }
    ...
    data.data.forEach((bucket, bucketIndex) => {
      if (isDebugMode()) {
        ...
        console.log('  - Time range:', bucket.start_time_iso, 'to', bucket.end_time_iso);
        ...
        bucket.results.forEach((result, resultIndex) => {
          ...
          console.log('    - Total tokens:', result.input_tokens + result.output_tokens);
...
export async function reconcileUsage(
  ...
  if (isNaN(dateObj.getTime())) {
    throw new Error(`Invalid date: "${targetDate}". Expected format: YYYY-MM-DD`);
  }
  ...
  try {
    ...
    for (const result of results) {
      ...
      if (isDebugMode()) {
        console.log(`Processing: ${result.model} → ${tier} tier (${totalTokens} tokens)`);
      }

      tierTotals[tier] += totalTokens;

      if (!modelBreakdown[result.model]) {
        modelBreakdown[result.model] = { tier, tokens: 0 };
      }
      modelBreakdown[result.model].tokens += totalTokens;
    ...
    if (isDebugMode()) {
      ...
      for (const [model, data] of Object.entries(modelBreakdown)) {
        console.log(`  ${model} (${data.tier}): ${data.tokens.toLocaleString()} tokens`);
      }

      console.log('\n=== Tier Totals Comparison ===');
      console.log('Premium Tier:');
      console.log('  - OpenAI reports:', tierTotals.premium.toLocaleString(), 'tokens');
      console.log('  - Our database has:', premiumBefore.tokens_used.toLocaleString(), 'tokens');
      console.log('  - Difference:', (tierTotals.premium - premiumBefore.tokens_used).toLocaleString(), 'tokens');

      console.log('\nMini Tier:');
      console.log('  - OpenAI reports:', tierTotals.mini.toLocaleString(), 'tokens');
      console.log('  - Our database has:', miniBefore.tokens_used.toLocaleString(), 'tokens');
      console.log('  - Difference:', (tierTotals.mini - miniBefore.tokens_used).toLocaleString(), 'tokens');
    }

    // Calculate the difference (what we missed)
    const premiumDiff = Math.max(0, tierTotals.premium - premiumBefore.tokens_used);
    const miniDiff = Math.max(0, tierTotals.mini - miniBefore.tokens_used);

    // Update our database with the difference
    if (premiumDiff > 0) {
      console.log(`Adding ${premiumDiff.toLocaleString()} tokens to premium tier`);
    ...
    // Get updated values
    const premiumAfter = getUsageRecord('premium')!;
    const miniAfter = getUsageRecord('mini')!;

    const totalAdded = premiumDiff + miniDiff;
    if (totalAdded > 0) {
      console.log(`Reconciliation complete: Added ${totalAdded.toLocaleString()} tokens`);
    } else {
