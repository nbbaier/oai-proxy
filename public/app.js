...
async function fetchUsageStats() {
  try {
    ...
    // Update date
    document.getElementById('current-date').textContent = `Updated: ${new Date().toLocaleString()} (${data.date})`;
...
function updateTierDisplay(tier, data) {
  ...
  // Update numbers
  document.getElementById(`${tier}-used`).textContent = data.used.toLocaleString();
  document.getElementById(`${tier}-limit`).textContent = data.limit.toLocaleString();
  document.getElementById(`${tier}-percentage`).textContent = `${percentage.toFixed(1)}%`;
